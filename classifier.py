...
def main():
    # Training:
    start = time.time()
    training_names, training_data = readNameTfIdf('assignment1_2017S1/training_data.csv', 1)
    labels = readCsvFile('assignment1_2017S1/training_labels.csv')
    end = time.time()
    ...
    # Make prediction:
    start = time.time()
    testing_names, testing_data = readNameTfIdf('assignment1_2017S1/test_data.csv', 0)
    end = time.time()
    print('Finish loading, the time used was: {0} seconds'.format(end - start))
    start = time.time()
